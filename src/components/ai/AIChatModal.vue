      ...
      <div class="ai-chat-content" ref="chatContent">
        ...
        <div v-for="(message, index) in history" :key="index" 
          ...
          <div class="ai-message-content">
            ...
            <!-- CAD Model Preview -->
            <div v-if="message.cadModel" class="ai-cad-preview">
              <div class="preview-header d-flex justify-content-between align-items-center">
                <span>{{ message.cadModel.name }}</span>
                <button class="btn btn-sm btn-primary" @click="openModelViewer(message.cadModel)">
                  3D Görünüm <i class="bi bi-box-arrow-up-right ms-1"></i>
                </button>
              </div>
              <div class="preview-image" @click="openModelViewer(message.cadModel)">
                <img :src="message.cadModel.previewImage || '/assets/images/models/placeholder.png'" alt="CAD Model">
            ...
            <div v-if="message.predictions && message.predictions.length > 0" class="ai-prediction-results">
              <div class="prediction-header">
                <span>Tahmin Sonuçları ({{ message.predictionType || 'Genel' }})</span>
                <span class="prediction-confidence">Güven Oranı: {{ formatConfidence(message.confidence) }}</span>
              </div>
              <div class="prediction-list">
                <div v-for="(pred, i) in message.predictions.slice(0, 3)" :key="i" class="prediction-item">
                  <div class="prediction-item-title">{{ pred.title || pred.name }}</div>
                ...
                <button 
                  v-if="message.predictions.length > 3" 
                  class="btn btn-sm btn-outline-primary mt-2"
                  @click="openPredictionDetails(message)"
                >
                  Tüm sonuçları göster ({{ message.predictions.length }})
                </button>
              </div>
            </div>
        ...
        <form @submit.prevent="handleSendMessage">
          ...
          <div v-if="showAdvancedOptions" class="advanced-options">
            <div class="row g-3 mt-2">
              ...
              <div class="col-md-6">
                <div class="form-check">
                  <input class="form-check-input" type="checkbox" v-model="options.runPrediction" id="predictionOption">
                  <label class="form-check-label" for="predictionOption">Tahmin analizi çalıştır</label>
                </div>
              </div>
            </div>
            <div class="row g-3 mt-2">
              ...
              <div class="col-md-6">
                <select class="form-select form-select-sm" v-model="selectedModelType" :disabled="!options.searchCADModels">
                  ...
                  <option value="TR36AI">Akım Trafosu</option>
                </select>
              </div>
            </div>
          </div>
...
const { sendMessage, history, isProcessing, clearHistory, supportedModels, switchModel, getCurrentModel } = aiService
...
const handleSendMessage = async () => {
  ...
  watch(history, scrollToBottom, { deep: true, once: true })
  ...
  try {
    ...
    if (shouldSearchTechnical.value) {
      const technicalResponse = await aiService.queryTechnical(userMessage)
      if (technicalResponse.relatedDocs && technicalResponse.relatedDocs.length > 0) {
        const lastMessageIndex = history.value.length - 1
        if (lastMessageIndex >= 0) {
          history.value[lastMessageIndex].relatedDocs = technicalResponse.relatedDocs
        }
      }
    }
    
    // CAD modelleri ara
    if (shouldSearchCADModels.value) {
      processCADModelRequest(userMessage)
    }
    
    // Tahmin analizi çalıştır
    if (shouldRunPrediction.value) {
      processPredictionRequest(userMessage)
    }
...
const processCADModelRequest = async (query) => {
  ...
  try {
    ...
    const model = systemData.cadModels.find(m => m.id === modelId)
    
    if (model) {
      // Mesaj geçmişine ekle
      history.value.push({
        role: 'assistant',
        content: `"${model.name}" 3D modelini incelemeye hazırım. 3D görünümü açmak için modele tıklayabilirsiniz.`,
        cadModel: model,
        timestamp: new Date()
      })
      
      // Modeli analiz et
      const analysis = await aiService.analyzeCADModel(modelId)
      if (analysis.success) {
        // Analiz sonucunu mesaj olarak ekle
        history.value.push({
          role: 'assistant',
          content: `${model.name} modeli hakkında teknik bilgiler:\n${analysis.data.description || 'Teknik bilgi bulunamadı.'}`,
          source: 'CAD Analizi',
          timestamp: new Date()
        })
      }
...
const processPredictionRequest = async (query) => {
  const lowerQuery = query.toLowerCase()
  let predictionType = 'production'
  let timeframe = 'nextWeek'
  
  // Tahmin türünü ve zaman çerçevesini belirle
  if (lowerQuery.includes('stok') || lowerQuery.includes('malzeme') || lowerQuery.includes('envanter')) {
    predictionType = 'inventory'
  } else if (lowerQuery.includes('hata') || lowerQuery.includes('arıza') || lowerQuery.includes('kalite')) {
    predictionType = 'defect'
  }
  
  if (lowerQuery.includes('ay')) {
    timeframe = 'nextMonth'
  } else if (lowerQuery.includes('çeyrek') || lowerQuery.includes('3 ay')) {
  ...
  try {
    ...
    switch (predictionType) {
      case 'inventory':
        predictionResult = await aiService.predictInventoryNeeds({ timeframe })
        break
      case 'defect':
        predictionResult = await aiService.predictDefects('all', { timeframe })
        break
      default:
        predictionResult = await aiService.predictProduction({ timeframe })
    }
    ...
    if (predictionResult.success && predictionResult.predictions) {
      ...
      history.value.push({
        role: 'assistant',
        content: `${getPredictionTypeTitle(predictionType)} tahmini oluşturuldu. Analiz sonuçları:`,
        predictions: predictionResult.predictions,
        predictionType: getPredictionTypeTitle(predictionType),
    ...
    console.error('Tahmin işleme hatası:', error)
  }
}

// Tahmin türü başlığını al
const getPredictionTypeTitle = (type) => {
  const titles = {
    'production': 'Üretim',
    'inventory': 'Stok',
    'defect': 'Kalite',
...
const openPredictionDetails = (message) => {
  selectedPredictions.value = message.predictions
  selectedPredictionType.value = message.predictionType
  selectedConfidence.value = message.confidence
}
